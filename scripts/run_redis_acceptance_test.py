...
def run_phase_1_users() -> None:
    with _evidence().phase("phase-1-users"):
        ...
        safe_email = expired_email.replace("'", "''")
        fixture_result = _runtime().postgres_sql(
            "UPDATE invite_tokens SET created_at = NOW() - INTERVAL '31 days' "
            f"WHERE user_email = '{safe_email}' AND is_used = 0 AND is_revoked = 0; "
            f"SELECT COUNT(*) FROM invite_tokens WHERE user_email = '{safe_email}' AND created_at < NOW() - INTERVAL '30 days';"
        )
        _require(fixture_result.splitlines()[-1:] == ["1"], "expiry fixture did not adjust exactly one pending invite")
...
def _acceptance_ssh_private_key() -> str:
    """Read only the run-scoped private key generated by acceptance_stack.sh."""
    key_path = Path(os.environ.get("TMPDIR", "/tmp")) / "platformops-redis-acceptance-runtime" / RUN_ID / "ssh_fixture_key"
    _require(key_path.is_file(), f"acceptance SSH fixture key is missing: {key_path}")
