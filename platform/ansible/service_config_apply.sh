...
if [[ -z "$RESOLVED_CONFIG_PATH" ]]; then
  FALLBACK_PATH="$(docker exec "$CONTAINER_NAME" find /iktara /fintrady /PlatformOps -type f \( -iname "*config*.yaml" -o -iname "*config*.yml" \) 2>/dev/null | grep -v "test" | head -n 1 || true)"
