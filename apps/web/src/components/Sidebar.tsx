...
export function Sidebar({ activeView, onViewChange }: SidebarProps) {
  return (
    <aside className="sidebar">
      ...
      >
        <svg className="ico" viewBox="0 0 24 24"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><path d="M22 6l-10 7L2 6"/></svg>
        Services
      </button>
      ...
      <button 
        className={`sb-item ${activeView === "node-metrics" ? "active" : ""}`}
        onClick={() => onViewChange("node-metrics")}
      >
        <svg className="ico" viewBox="0 0 24 24"><rect x="2" y="3" width="20" height="14" rx="2"/><path d="M8 21h8M12 17v4"/></svg>
        Node Metrics
      </button>
      <button 
        className={`sb-item ${activeView === "observability-stack" ? "active" : ""}`}
        onClick={() => onViewChange("observability-stack")}
      >
        <svg className="ico" viewBox="0 0 24 24"><path d="M14.7 6.3a1 1 0 000 1.4l1.6 1.6a1 1 0 001.4 0l3.77-3.77a6 6 0 01-7.94 7.94l-6.91 6.91a2.12 2.12 0 01-3-3l6.91-6.91a6 6 0 017.94-7.94l-3.76 3.76z"/></svg>
        Observability Stack
      </button>
